...
- [x] 3.2 Manually verify in the dev server: from the consolidated view of a non-current month, click "Nova compra parcelada", submit with a purchase date in a different month, and confirm the app redirects back to the original (non-current) month, not today's month or the purchase month
- [x] 3.3 Manually verify the parcelas appear correctly in the resulting consolidated view (or in the months where they actually land, per existing parcela-month logic)
